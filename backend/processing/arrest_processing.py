...
# File paths
INPUT_FILE = "data/distilled_data/articles.csv"
OUTPUT_FILE = "data/distilled_data/arrests.csv"
...
def extract_location_with_deepseek(title: str, content: str) -> Dict:
    ...
    prompt = f"""
You are a geographic location extraction expert. Analyze the following article about an ICE arrest and extract the PRIMARY location where the arrest most likely occurred.

Article Title: {title}
Article Content: {content}

Extract the following information about the PRIMARY location (where the arrest actually happened):
...
Focus on the PRIMARY location where the arrest event occurred, not secondary locations mentioned in the article.
"""
    ...
    try:
        ...
        try:
            ...
            required_fields = ["location", "latitude", "longitude", "city", "state", "county", "zip", "street"]
...
def process_arrest_articles():
    """Main function to process arrest articles and generate arrests.csv."""
    
    if not DEEPSEEK_API_KEY:
        raise ValueError("DEEPSEEK_API_KEY environment variable is required")
    
    # Read arrest articles from input file
    arrest_articles = []
    ...
    try:
        with open(INPUT_FILE, 'r', encoding='utf-8') as f:
            ...
            for row in reader:
                if row.get('category', '').lower() == 'arrest':
                    arrest_articles.append(row)
    ...
    logger.info(f"Found {len(arrest_articles)} arrest articles to process")
    
    # Process each article
    processed_arrests = []
    
    with tqdm(total=len(arrest_articles), desc="Processing arrest articles", unit="article") as pbar:
        for article in arrest_articles:
            try:
                ...
                arrest_record = {
                    ...
                    'reported_count': '1',  # Default to 1 arrest per article
                    'url': article['url']
                }
                
                processed_arrests.append(arrest_record)
                
                
            except Exception as e:
                logger.error(f"Error processing article {article.get('id', 'unknown')}: {e}")
                # Create a record with unknown values
                arrest_record = {
    ...
    # Write output CSV
    fieldnames = ['id', 'location', 'latitude', 'longitude', 'city', 'state', 'county', 'zip', 'street', 'reported_count', 'url']
    
    try:
        with open(OUTPUT_FILE, 'w', newline='', encoding='utf-8') as f:
            writer = csv.DictWriter(f, fieldnames=fieldnames)
            writer.writeheader()
            writer.writerows(processed_arrests)
        
        logger.info(f"Successfully processed {len(processed_arrests)} arrest articles")
...
def main():
    """Main entry point."""
    logger.info("Starting arrest processing...")
    process_arrest_articles()
    logger.info("Arrest processing completed.")
